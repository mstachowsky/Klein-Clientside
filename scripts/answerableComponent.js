...
Not quite ready for this.  We need to create a single HTML element to hold a bunch of Radio buttons, then we need to figure out which one is active for the check answer.  This will be a later project
class multiAnsBox extends answer{
	constructor(dataString,newID=""){
		super(dataString,"text");
		this.ID = newID;
	}
	...
	ansBx.setAttribute("id",that.ID+"_answerBox");
	ansBx.setAttribute("class","multiAnsBox");
	ansBx.onchange=function(){that.setAnsString(ansBx.value)};
