...
function printBook() {
	...
	for (var i = 0; i < pageNum; i++) {
		...
		pageContentMD = parseMD(pageContentMD);
		
		console.log("pageContentMD = " + pageContentMD)
		...
		while (replaceStackCopyMD.length > 0) {
			...
			pageContentMD += replaceStackCopyMD.shift();
		}
	...
	var msg = window.prompt("Write the name of your Book", "");
	contentMD += "<bookName>" + msg;
	const options = {
		method: 'POST',
		headers: {
...
function parseMD(pageContent) {
	...
	pageContent = pageContent.replaceAll("</h3>", "");
	pageContent = pageContent.replaceAll("</h3>", "");
	//pageContent = pageContent.replaceAll(/<img(.*?)\">/g, "");
	pageContent = pageContent.replaceAll(/<div(.*?)\">/g, "");
	pageContent = pageContent.replaceAll("<br>", "\n");
	// pageContent = pageContent.replaceAll("data-vscid=\"","");
	// <a href="link.com">text</a>
	console.log("pageContentMDend = " + pageContent)
	return pageContent;
}
