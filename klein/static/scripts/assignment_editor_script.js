...
function newPage() {
	...
	pageNameText.innerHTML = "Question Name: ";
...
function collapse(currPage) {
	...
	if (document.getElementById("page" + currPage).classList.contains("questionGroup")) {
		...
		} else if (pageArr[0] && pageArr[0].style.display == 'none') {
			for (var i = 0; i < pageArr.length; i++) {
				pageArr[i].style.display = 'block'
			}
		} else if (pageArr[0] && pageArr[0].style.display == 'block') {
			for (var i = 0; i < pageArr.length; i++) {
				pageArr[i].style.display = 'none'
			}
		}
...
function printBook() {
	...
	contentMD += "!assignmentVariables\n\n";

	content += variablePrint()
	contentMD += variablePrintMD()

	content += "!endAssignmentVariables<br>";
	contentMD += "!endAssignmentVariables\n";

	content += "!Assignment " + document.getElementById("bookNameIn").value + "<br>";
	contentMD += "!Assignment " + document.getElementById("bookNameIn").value + "\n";
	...
	for (var i = 0; i < pageNum; i++) {
		...
		if (page.classList.contains("questionGroup")) {
			...
			contentMD += "!qGroup " + document.getElementById("qGroupNameIn" + i).value + "\n"
			...
			for (var j = 0; j < pageArr.length; j++) {
				if (pageArr[j].id.startsWith("pageInputText")) {
					...
					var pageContentMD = pageArr[j].outerHTML

					pageContent = parse(pageContent);
					pageContentMD = parse(pageContentMD);

					var replaceStackCopy = [...replaceStack]
					var replaceStackCopyMD = [...replaceStackMD]
					...
					while (replaceStackCopyMD.length > 0) {
						pageContentMD = pageContentMD.replace(pageContentMD, replaceStackCopyMD.shift());
					}

					content += pageContent
					contentMD += pageContentMD + "\n"
					content += "!endQuestion<br>"
					contentMD += "!endQuestion\n"
				}
			...
			contentMD += "!Question " + document.getElementById("pageNameIn" + i).value + "\n"
			var pageContent = document.getElementById("pageInputText" + i).outerHTML
			var pageContentMD = document.getElementById("pageInputText" + i).outerHTML
			pageContent = parse(pageContent);
			pageContentMD = parse(pageContentMD);
			var replaceStackCopy = [...replaceStack]
			var replaceStackCopyMD = [...replaceStackMD]
	...
	var msg = window.prompt("Write the name of your Assignment", "");
	contentMD += "<assignmentName>" + msg;
	const options = {
		method: 'POST',
		headers: {
	...
	// fetch('http://172.31.218.126:5000/assignmentEditor/', options)
		...
		.then(text => {
			...
			if (text == "[WARNING]: overwrite") {
				let confirmAction = confirm("[WARNING] - File already exists! Are you sure to execute this action?");
				if (confirmAction) {
					const options = {
						method: 'POST',
						headers: {
...
function parseMD(pageContent) {
	...
	pageContent = pageContent.replaceAll("<i>", "*");
	pageContent = pageContent.replaceAll("</i>", "*");
	pageContent = pageContent.replaceAll("<hr>", "---\n");
	pageContent = pageContent.replaceAll("<ol>", "!oList");
	pageContent = pageContent.replaceAll("<ul>", "!list");
	pageContent = pageContent.replaceAll("</ol>", "\n!endList\n");
	pageContent = pageContent.replaceAll("</ul>", "\n!endList\n");
	pageContent = pageContent.replaceAll("<li>", "\n!item ");
	pageContent = pageContent.replaceAll("class=\"pageInputText\"", "");
	pageContent = pageContent.replaceAll("<h1>", "# ");
	pageContent = pageContent.replaceAll("</h1>", "");
	pageContent = pageContent.replaceAll("<h2>", "## ");
...
function multipleChoice() {
	...
	var questionIn = document.createElement("input")
	questionIn.setAttribute("type", "text")
	questionIn.setAttribute("id", "MCScore");
	form.appendChild(questionIn)
...
function MCSubmit() {
	...
	var score = document.getElementById("MCScore").value
...
function ansBoxRadio(abs, tol, text, pageInNum) {
	...
	absForm.appendChild(score)

	var scoreIn = document.createElement("input")
	scoreIn.setAttribute("type", "text")
	scoreIn.setAttribute("id", "absScoreIn");
	absForm.appendChild(scoreIn)
	...
	var score = document.createElement("label")
	score.innerHTML = "Score: "
	tolForm.appendChild(score)

	var scoreIn = document.createElement("input")
	scoreIn.setAttribute("type", "text")
	scoreIn.setAttribute("id", "tolScoreIn");
	tolForm.appendChild(scoreIn)
	...
	var score = document.createElement("label")
	score.innerHTML = "Score: "
	textForm.appendChild(score)

	var scoreIn = document.createElement("input")
	scoreIn.setAttribute("type", "text")
	scoreIn.setAttribute("id", "textScoreIn");
	textForm.appendChild(scoreIn)
...
function ansBoxSub(abs, tol, text, pageInNum) {
	...
	if (abs) {
		...
		var score = document.getElementById("tolScoreIn").value
		document.getElementById("tolIDIn").focus()
		var tol = document.getElementById("toltolIn").value;
		var ab = "!ans numeric:" + val + ":" + tol + " " + id + "<br>!score " + score + "<br>!feedback<br>!endFeedback"
		// page.innerHTML = page.innerHTML + ab
		// page.removeChild(document.getElementById("ABForm"));
	} else if (text) {
		var textIn = document.getElementById("textIn").value;
		document.getElementById("textIn").focus()
		var score = document.getElementById("textScoreIn").value
		var id = document.getElementById("textIDIn").value;
		var ab = "!ans " + textIn + " " + id + "<br>!score " + score + "<br>!feedback<br>!endFeedback"
		// page.innerHTML = page.innerHTML + ab
		// page.removeChild(document.getElementById("ABForm"));
	}
...
function image() {
	...
	form.addEventListener('drop', (event) => {
		...
		document.getElementById("imgIn").value = "res/" + fileList[0].name;
...
function submitImage() {
	...
	//fetch('http://172.31.218.126:5000/assignmentEditor/', options)
		.then(res => {
			...
			bookPath = "../static/BOOKS/" // Need to change this path to appropriate folder path.
			...
			// var imgPrint = "!img " + alt + " " + width + " " + height + " " + img
			var imgPrint = "!img " + alt + " " + img

			//Appends the replacements to be replaced when printed
			replaceStack.push(imgHtml.outerHTML)
			replaceStack.push(imgPrint)

			replaceStackMD.push(imgPrint)
...
function video() {
	...
	thumIn.addEventListener('dragover', (event) => {
		event.stopPropagation();
		event.preventDefault();
		event.dataTransfer.dropEffect = 'copy';
	});

	thumIn.addEventListener('drop', (event) => {
		event.stopPropagation();
		event.preventDefault();
		const fileList = event.dataTransfer.files;
		console.log(fileList[0]);

		document.getElementById("thumIn").value = "res/" + fileList[0].name;

		formDataVideo.append("image", fileList[0], fileList[0].name);
	});

	vidIn.addEventListener('dragover', (event) => {
		event.stopPropagation();
		event.preventDefault();
		event.dataTransfer.dropEffect = 'copy';
	});

	vidIn.addEventListener('drop', (event) => {
		event.stopPropagation();
		event.preventDefault();
		const fileList = event.dataTransfer.files;
		console.log(fileList[0]);

		document.getElementById("vidIn").value = "res/" + fileList[0].name;

		formDataVideo.append("video", fileList[0], fileList[0].name);
	});
...
function submitVideo() {
	...
	//fetch('http://172.31.218.126:5000/assignmentEditor/', options)
		.then(res => {
			...
			// var vidText = "!video " + alt + " " + thum + " " + vid;
			var vidText = "!video " + alt + " " + vid;
...
function variablePrintMD() {
	...
	if (!(variables.startsWith("<div>"))) {
		variables = "!var " + variables
		variables = variables.replace("<div>", "\n!var ")
	}

	variables = variables.replaceAll("<div>", "!var ")
	variables = variables.replaceAll("</div>", "\n")
	variables += "\n"
	return variables
}
...
function questionGroup() {
	...
	qGroup.setAttribute("class", "questionGroup")
	qGroup.setAttribute("id", "page" + pageNum);

	var qGroupText = document.createElement("label");
	qGroupText.innerHTML = "Question Group Name: ";
	qGroup.appendChild(qGroupText);

	var qGroupTextIn = document.createElement("input");
	qGroupTextIn.setAttribute("type", "text");
	qGroupTextIn.setAttribute("id", "qGroupNameIn" + pageNum);
	qGroup.appendChild(qGroupTextIn);

	var pageCol = document.createElement("button")
	pageCol.innerHTML = "Collapse"
	pageCol.setAttribute("onClick", "collapse(" + pageNum + ")")
	pageCol.setAttribute("class", "collapseButton")
	qGroup.appendChild(pageCol)

	var newQuesBut = document.createElement("button")
	newQuesBut.innerHTML = "New Question"
	newQuesBut.setAttribute("onClick", "newQuestionQGroup(" + pageNum + ")")
	newQuesBut.setAttribute("class", "collapseButton")
	qGroup.appendChild(newQuesBut)

	var newQuesBut = document.createElement("button")
	newQuesBut.innerHTML = "Remove Last Question"
	newQuesBut.setAttribute("onClick", "removeLastQuesQGroup(" + pageNum + ")")
	newQuesBut.setAttribute("class", "collapseButton")
	qGroup.appendChild(newQuesBut)
	...
	var pageAppend = document.getElementById("input")
	pageAppend.appendChild(qGroup)
}

function newQuestionQGroup(currPage) {
	var qGroup = document.getElementById("page" + currPage)

	var pageArr = document.getElementById("page" + currPage).getElementsByTagName("div")
	if (pageArr[0] && pageArr[0].style.display == 'none') {
		collapse(currPage)
	}
	...
	var pageIn = document.createElement("div");
	pageIn.setAttribute("contenteditable", "true");
	pageIn.setAttribute("id", "pageInputText" + currPage);
	pageIn.setAttribute("class", "pageInputText");
	qGroup.appendChild(pageIn);
}

function removeLastQuesQGroup(currPage) {
	var qGroup = document.getElementById("page" + currPage)
	var pageArr = document.getElementById("page" + currPage).getElementsByTagName("div")

	if (pageArr.length > 0) {
		qGroup.removeChild(pageArr[pageArr.length - 1])
	}
}
